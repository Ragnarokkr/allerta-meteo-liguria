...
function getAlert(dom) {
  ...
  const query = evalute(dom, '//section[3]/div/a/div');
...
function getRisk(dom) {
  ...
  const query = evalute(dom, '//section[3]/div/a/div/h1');
  if (query.snapshotLength == 0) return 'unknown';
...
function getInfo(dom) {
  ...
  const query = evalute(dom, '//section[3]/div/a/div/h2[2]');
  if (query.snapshotLength == 0) return 'unknown';
  return query.snapshotItem(0).innerText.toLocaleLowerCase() ?? 'unknown';
}
function getDate(dom) {
  // XPath: /html/body/section[3]/div/a/div/h2[1]
  const query = evalute(dom, '//section[3]/div/a/div/h2[1]');
...
function getLinks(dom) {
  ...
  const alertBulletin = evalute(dom, '//section[5]/div/div/div/div[2]/div/div[6]/div/a');
  ...
  const forecastBulletin = evalute(dom, '//section[5]/div/div/div/div[3]/div/div[4]/div/a');
  ...
  const alertMap = evalute(dom, '//section[4]/div/div[1]/div/div[2]/div/img');
